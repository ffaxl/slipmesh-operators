...
/// Same as `error_chain`, but for `anyhow::Error` specifically - it deliberately doesn't
/// implement `std::error::Error` itself, so can't be passed to `error_chain` as a trait object.
/// Anyhow's own alternate-`Display` (`{:#}`) already renders its whole context chain the same
/// single-line way.
