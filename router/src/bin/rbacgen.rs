...
//! RouterPool/RouterConfig - since MeshLink/MeshNode/RoadWarrior are mesh's/roadwarriors' to
//! define), this prints rules for every resource router's *binary* actually touches, ownership
//! aside: it reads MeshLink/MeshNode/RoadWarrior just as much as it reads its own CRDs, and needs
//! RBAC for all of it to run. `routerpools`/`routerpools/status` go through
//! `slipmesh_core::pool::allocate`/`release`, whose status write is `replace_status` (HTTP PUT ->
//! the `update` verb, not `patch`).
...
use slipmesh_core::router_types::{BypassSource, RouterConfig, RouterNode, RouterPool};
